...
export class CommandesService {
  ...
  // ── Statistiques dashboard (agregation serveur) ────────────────────────────
  // Calcule le CA, le nombre de commandes, le panier moyen, les abonnements
  // actifs, la serie de CA par periode, le CA par categorie et le top produits.
  // Fait entierement en base (aggregation) sur TOUTES les commandes payees :
  // corrige le calcul cote client limite a 1000 commandes.
  async getStats(period = '7d') {
    try {
      const buckets = this.buildStatBuckets(period);
      const rangeStart = buckets[0].start;
      const rangeEnd = buckets[buckets.length - 1].end;

      // Serie de CA : commandes PAID de la periode, regroupees par bucket.
      const paidInRange = await this.commandeModel
        .find(
          {
            statut: StatutCommande.PAID,
      ...
      for (const cmd of paidInRange) {
        const t = new Date((cmd as any).createdAt).getTime();
        for (let i = 0; i < buckets.length; i++) {
          if (t >= buckets[i].start.getTime() && t < buckets[i].end.getTime()) {
            series[i].value += Number((cmd as any).totalPrice ?? 0);
            break;
          }
        }
      }
      series.forEach((s) => (s.value = Math.round(s.value * 100) / 100));

      const periodRevenue =
        Math.round(series.reduce((acc, s) => acc + s.value, 0) * 100) / 100;
      const periodOrders = paidInRange.length;
      const avgBasket =
        periodOrders > 0
          ? Math.round((periodRevenue / periodOrders) * 100) / 100
          : 0;

      // CA total (toutes periodes) + nombre total de commandes payees.
      const [totals] = await this.commandeModel.aggregate([
        { $match: { statut: StatutCommande.PAID } },
        {
          $group: {
            _id: null,
      ...
      const totalRevenue = Math.round((totals?.revenue ?? 0) * 100) / 100;
      const totalOrders = totals?.count ?? 0;

      // Abonnements actifs (sur toutes les commandes).
      const [activeAgg] = await this.commandeModel.aggregate([
        { $unwind: '$abonnements' },
        { $match: { 'abonnements.statut': StatutAbonnement.ACTIVE } },
        { $count: 'total' },
      ]);
      const activeSubscriptions = activeAgg?.total ?? 0;

      // CA par categorie + top produits : on deroule les abonnements des
      // commandes PAID et on remonte produit → service → categorie.
      const byProduct = await this.commandeModel.aggregate([
        { $match: { statut: StatutCommande.PAID } },
        { $unwind: '$abonnements' },
        {
          $group: {
            _id: '$abonnements.product',
            revenue: {
              $sum: {
                $multiply: [
                  { $ifNull: ['$abonnements.price', 0] },
                  { $ifNull: ['$abonnements.quantity', 1] },
                ],
              },
            },
          },
        ...
        { $unwind: { path: '$category', preserveNullAndEmptyArrays: true } },
        {
          $project: {
            ...
            productName: { $ifNull: ['$product.name', 'Inconnu'] },
      ...
      return ApiResponse.error('Impossible de calculer les statistiques');
    }
  }

  // Construit des buckets de dates alignes sur le PeriodSelector du dashboard
  // ('7d' = 7 jours, '5w' = 5 semaines, '30d' = 10x3 jours, 'month' = 4 semaines).
  private buildStatBuckets(period: string) {
    ...
    const push = (start: Date, days: number, label: string) => {
    ...
    if (period === '5w') {
      ...
      for (let i = 9; i >= 0; i--) {
        ...
        push(
          start,
          3,
          start.toLocaleDateString('fr-FR', { month: 'short', day: 'numeric' }),
        );
      }
